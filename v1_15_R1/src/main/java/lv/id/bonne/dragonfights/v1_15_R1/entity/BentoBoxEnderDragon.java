...
package lv.id.bonne.dragonfights.v1_15_R1.entity;
...
import lv.id.bonne.dragonfights.v1_15_R1.entity.controllers.CustomControllerManager;
import lv.id.bonne.dragonfights.v1_15_R1.entity.controllers.CustomControllerPhase;
import lv.id.bonne.dragonfights.v1_15_R1.entity.controllers.ICustomController;
	...
	public void movementTick()
	{
		...
		else
		{
			...
			if (vec3d1 != null)
			{
				...
				this.bB *= 0.8F;
				this.bB = (float) ((double) this.bB + d6 * (double) idragoncontroller.getRotation());
			...
			this.bz = this.intersect(this.children[0].getBoundingBox()) |
		...
		else
		{
			...
			else
			{
				if (damageSource.getEntity() instanceof EntityHuman || damageSource.isExplosion())
				{
					...
					if (this.isFrozen() && !this.getCustomControllerManager().getCurrentPhase().isLanded())
					{
						this.setHealth(1.0F);
	...
	private void applyKnockBack(List<Entity> list)
	{
		...
		while (iterator.hasNext())
		{
			...
			if (entity instanceof EntityLiving)
			{
				...
				entity.h(xOffset / squaredDistance * 4.0D, 0.20000000298023224D, zOffset / squaredDistance * 4.0D);

				if (!this.getCustomControllerManager().getCurrentPhase().isLanded() && ((EntityLiving) entity).cI() < entity.ticksLived - 2)
				{
