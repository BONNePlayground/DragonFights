...
public class NMSEntityRegistry extends HashCustomRegistry {
    ...
    public void applyRegister(CustomEntityRoot<? extends Entity> type) {
        ...
        try {
            Constructor<? extends net.minecraft.server.v1_16_R2.Entity> constructor = (Constructor<? extends net.minecraft.server.v1_16_R2.Entity>) type.getNMSClass(ServerVersion.v1_16_2).getConstructor(EntityTypes.class, World.class);
            ...
            IRegistry.a(IRegistry.ENTITY_TYPE, type.getBaseID(ServerVersion.v1_16_2), type.getKey(), customEntityNMSEntityType);
            ...
            if(type.isVanilla(ServerVersion.v1_16_2))
                overrideSpawn(nmsCreatureType, baseKey, customEntityNMSEntityType);

            for(BiomeSpawn spawn : type.getSpawns(ServerVersion.v1_16_2))
    ...
    private void registerSpawn(EnumCreatureType creatureType, EntityTypes<? extends net.minecraft.server.v1_16_R2.Entity> customType, BiomeSpawn spawn) {
        Stream.of(spawn.getBiomes())
            .map(Biome::name)
            .map(String::toLowerCase)
            .map(MinecraftKey::a)
            .map(RegistryGeneration.WORLDGEN_BIOME::get)
            .filter(Objects::nonNull)
            .forEach(biomeBase -> {
                List<BiomeSettingsMobs.c> meta = biomeBase.b().a(creatureType);
                if(meta == null || meta.isEmpty())
                    return;

                meta.add(new BiomeSettingsMobs.c(customType, spawn.getWeight(), spawn.getMin(), spawn.getMax()));
            });
    }

    private void overrideSpawn(EnumCreatureType creatureType, MinecraftKey baseKey, EntityTypes<? extends net.minecraft.server.v1_16_R2.Entity> customType) {
        RegistryGeneration.WORLDGEN_BIOME.g()
            .map(biomeBase -> biomeBase.b().a(creatureType))
            .filter(Objects::nonNull)
            .flatMap(List::stream)
            .filter(meta -> Objects.equals(((IRegistry<EntityTypes<? extends net.minecraft.server.v1_16_R2.Entity>>) IRegistry.ENTITY_TYPE).getKey(meta.c), baseKey))
            .forEach((ThrowingConsumer<BiomeSettingsMobs.c>) biomeMeta -> biomeEntityType.set(biomeMeta, customType));
    }
}